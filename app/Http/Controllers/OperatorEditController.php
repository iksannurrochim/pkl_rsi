...
class OperatorEditController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        return view('edit_profil_operator.index', [
            'title' => 'Edit Profile'
        ])->with(compact('user'));
    }
    ...
    public function update(Request $request, string $id)
    {
        $request->validate([
            ...
            'nomor_id.required' => 'NIM / NIS harus diisi.',
        ...
        // Update to DB
        $user = User::where('nomor_id', $id)->first(); 
        $userData = [
        ...
        if ($request->hasFile('fileProfile')) {
            ...
            if ($user->foto) {
                $oldPath = public_path('files/Profile/' . $user->foto);
        ...
        DB::table('user')->where('nomor_id', $id)->update($userData);
